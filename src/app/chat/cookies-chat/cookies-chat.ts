...
export class CookiesChat {
  ...
  // ...
  // Creamos un control de formulario
  // Valor inicial: ''
  // Validadores: es 'required' (no puede estar vacio)
  messageControl = new FormControl('', [Validators.required]);
  // .. (el form control) ...

  // Signal para el estado de "cargando"
  loading = signal(false);

  // Signal para el historial del chat.
  // Es un array de objectos ChatMessage.
  ...
  async handleSubmit(): Promise<void> {
    ...
    if (this.messageControl.invalid) {
      return; // Si es invalido, no hacer nada
    }

    const userMessage = this.messageControl.value || '';

    // 2. Actualiza el estado con Signals
    ...
    try {
      // 4. Eviar el mensaje a Gemini
      const result = await this.chat.sendMessage(userMessage);
      const response = await result.response;
      let botResponse = response.text();

      // 5. Formatear la respuesta (como en tu codigo original)
      // (Nota: Esta no es la forma mas robusta, pero funciona para el demo)
      ...
      this.chatHistory.update((currentHistory) => [
        ...currentHistory,
        { parts: 'Ups, algo salio mal. Intenta de nuevo.', role: 'model' },
      ]);
    } finally {
